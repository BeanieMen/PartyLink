    ...
    return (
        <View style={[styles.messageBubbleContainer, isMyMessage ? styles.myMessageContainer : styles.otherMessageContainer]}>
            <View style={bubbleStyles}>
                {!isMyMessage && (
                    <Text style={senderNameStyles}>
                        {message.senderUser?.username || 'Unknown User'}
                    </Text>
                )}
...
const GroupChatScreen: React.FC = () => {
    ...
    const [chatSessionInfo, setChatSessionInfo] = useState<{session: ChatSessionRow, participants: UserRow[]} | null>(null);

    const flatListRef = useRef<FlatList>(null);
    const wsRef = useRef<WebSocket | null>(null);

    // No need for keyboardHeight state and listener, KeyboardAvoidingView handles it

    const fetchChatSessionInfo = useCallback(async () => {
        try {
            const response = await fetch(`${API_BASE_URL}/chat/${chatSessionId}`);
            if (response.ok) {
                const sessionData: {session: ChatSessionRow, participants: UserRow[]} = await response.json();
    ...
    useEffect(() => {
        if (isLoaded && isSignedIn && myUserId && chatSessionId) {
            ...
            const wsUrl = `${API_BASE_URL.replace('http', 'ws')}/ws/${chatSessionId}`;
