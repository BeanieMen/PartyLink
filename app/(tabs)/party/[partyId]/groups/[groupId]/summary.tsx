...
import {
  ...
  QuestionRow,
  VoteRow,
  CommentRow as OriginalCommentRow,
} from '@/types/database'
...
const GroupSummaryScreen: React.FC = () => {
  ...
  const fetchSummaryDetails = useCallback(async () => {
    ...
    try {
      ...
      if (!groupRes.ok)
        throw new Error(`Failed to fetch group: ${groupRes.statusText} (${groupRes.status})`)
      const groupData: BrowsableGroupApiResponse = await groupRes.json()

      if (!membersRes.ok)
        throw new Error(`Failed to fetch members: ${membersRes.statusText} (${membersRes.status})`)
      ...
      if (!questionsRes.ok)
        throw new Error(
          `Failed to fetch questions: ${questionsRes.statusText} (${questionsRes.status})`,
        )
      ...
      if (!commentsRes.ok)
        throw new Error(
          `Failed to fetch comments: ${commentsRes.statusText} (${commentsRes.status})`,
        )
      ...
      const formattedComments: GroupSummaryCommentsApiResponse[] = await Promise.all(
        rawCommentsData.map(async (c) => {
          try {
            ...
            if (!userRes.ok) {
              console.warn(
                `Failed to fetch user details for comment by ${c.user_id}: ${userRes.status}`,
              )
  ...
  const CustomHeader = ({ title }: { title: string }) => (
    <View style={headerStyles.headerContainer}>
      <TouchableOpacity
        onPress={() => router.replace(`/party/${partyId}/groups/explore`)}
        ...
        <View style={styles.summarySection}>
          ...
          {members.length > 0 ? (
            <ScrollView
              ...
              contentContainerStyle={styles.summaryMembersScroll}
            >
              {members.map((member) => (
                <View key={member.userId} style={styles.summaryMemberItemHorizontal}>
                  <Image
                    ...
                    onError={(e) =>
                      console.warn(
                        `Failed to load member portrait for ${member.userId}:`,
                        e.nativeEvent.error,
                      )
                    }
                  />
            ...
            comments.map((comment, cIndex) => (
              ...
              >
                <Image
                  ...
                  onError={(e) =>
                    console.warn(
                      `PFP load fail (commenter portrait): ${comment.user_id}`,
                      e.nativeEvent.error,
                    )
                  }
                />
