...
const PageColors = {
  background: "#100a26", cardBackground: "rgba(58, 31, 93, 0.85)", inputBackground: "rgba(255, 255, 255, 0.1)",
  textWhite: "#FFFFFF", textGray: "#A0A0A0", textGray300: "#D1D5DB", primary: "#7c3aed",
  accent: "#be185d", disabled: "#555", warningText: "#FFD700",
};
...
const GroupEditsScreen: React.FC = () => {
  ...
  useEffect(() => {
    if (!authLoaded) { setIsLoadingGroup(true); return; }
    if (!userId || !partyId) {
    ...
    const fetchUserGroup = async () => {
      ...
      try {
        ...
        if (response.status === 404) {
          ...
          setGroupDetails(null); setIsAuthorized(false);
          router.replace(`/party/${partyId}`);
          return;
        }
        if (!response.ok) {
          const errorData = await response.json().catch(() => ({ message: "Failed to fetch your group details." }));
  ...
  const fetchGroupMembersAndInvited = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ message: "Failed to fetch group member statuses" }));
        throw new Error(errorData.message);
      }
      ...
      setRequestedUserIds(groupMembers.members.filter(m => m.status === 'requested').map(m => m.userId));
    } catch (error) {
      console.error("Error fetching member statuses:", error);
      Alert.alert("Error", error instanceof Error ? error.message : "Could not load group member information.");
      setCurrentGroupUserIds([]); setInvitedUserIds([]); setRequestedUserIds([]);
    }
  ...
  const fetchGroupQuestions = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ message: "Failed to fetch questions for the group." }));
        throw new Error(errorData.message);
      }
      ...
      Alert.alert("Error", error instanceof Error ? error.message : "Could not load group questions.");
      setQuestions([]);
  ...
  const handleSearchUsers = useCallback(async () => {
    if (!groupDetails?.group_id || !isAuthorized) { setSearchResults([]); setIsSearchingUsers(false); return; }
    ...
    try {
      ...
      const filteredUsers = users.filter(u => u.username?.toLowerCase().startsWith(searchQuery.toLowerCase()));
      const formattedUsers: FoundUser[] = filteredUsers.map(u => ({
        ...u, isAlreadyMember: currentGroupUserIds.includes(u.user_id), isInvited: currentInvitedUserIds.includes(u.user_id)
      }));
      ...
      Alert.alert("Error", error instanceof Error ? error.message : "Could not perform user search.");
      setSearchResults([]);
    } finally { setIsSearchingUsers(false); }
  }, [groupDetails?.group_id, isAuthorized, searchQuery, partyId, userId, currentGroupUserIds, currentInvitedUserIds, requestedUserIds]);
  ...
  }, [searchQuery, groupDetails, isAuthorized, currentGroupUserIds, currentInvitedUserIds, requestedUserIds, handleSearchUsers]);
  ...
  const handleRespondToRequest = async (targetUserId: string, action: 'accept' | 'decline') => {
    ...
    try {
      ...
      Alert.alert("Error", error instanceof Error ? error.message : "Could not process the request.");
    } finally { setIsProcessingAction(null); }
  };
  ...
  const handleAddNewQuestion = () => {
    ...
    setQuestions(qs => [...qs, {
      question_id: newQuestionId, group_id: groupDetails.group_id, question_text: "", localText: "",
      order_index: qs.length, created_at: new Date().toISOString(), isNew: true,
    }]);
  };
  ...
  const handleSaveAllQuestionChanges = async () => {
    if (!groupDetails?.group_id || !isAuthorized || !initialQuestionsLoaded) {
      Alert.alert("Error", "Cannot save questions at this time. Ensure group is loaded, you are authorized, and questions were initially loaded.");
      return;
    }
    setIsSavingAll(true);
    try {
      ...
      if (originalQuestionsResponse.ok) {
        ...
        console.warn("Could not fetch original questions before saving. Deletions might be missed if state is inconsistent.");
      }
      ...
      const payload = {
        questionsToUpdate: questions.filter(q => !q.isNew).map((q) => {
          ...
          if (originalQ && (q.localText !== originalQ.question_text || currentOrderIndex !== originalQ.order_index)) {
            return { question_id: q.question_id, question_text: q.localText, order_index: currentOrderIndex };
          }
          return null;
        }).filter(item => item !== null),
        questionsToAdd: questions.filter(q => q.isNew && q.localText?.trim() !== "")
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ message: "Failed to save question changes." }));
        throw new Error(errorData.message);
      }
      ...
      <View style={styles.searchResultItem}>
        ...
        {isRequested && isAuthorized ? (
          <View style={styles.requestActionsContainer}>
            <TouchableOpacity style={styles.acceptRequestButton} onPress={() => handleRespondToRequest(item.user_id, 'accept')} disabled={isProcessingAction !== null}>
              {isProcessingThisUser && !isSavingAll ? <ActivityIndicator size="small" color={PageColors.textWhite} /> : <Text style={styles.requestButtonText}>Accept</Text>}
            </TouchableOpacity>
            <TouchableOpacity style={styles.declineRequestButton} onPress={() => handleRespondToRequest(item.user_id, 'decline')} disabled={isProcessingAction !== null}>
              {isProcessingThisUser && !isSavingAll ? <ActivityIndicator size="small" color={PageColors.textWhite} /> : <Text style={styles.requestButtonText}>Decline</Text>}
            </TouchableOpacity>
          </View>
          ...
          <TouchableOpacity style={styles.inviteButton} onPress={() => handleInviteUser(item.user_id)} disabled={isProcessingAction !== null}>
  ...
  const renderQuestionItem = ({ item, index }: { item: EditableQuestion, index: number }) => (
    <View style={styles.questionItem}>
      <TextInput
        style={styles.questionInput} value={item.localText} onChangeText={(text) => handleQuestionTextChange(item.question_id, text)}
        placeholder={`Question ${index + 1}`} placeholderTextColor={PageColors.textGray} multiline
      />
        ...
        return (
          <View style={styles.sectionCard}>
            ...
            <View style={styles.searchInputContainer}>
              <TextInput
                style={styles.searchInput} placeholder="Search by username..." placeholderTextColor={PageColors.textGray}
                value={searchQuery} onChangeText={setSearchQuery} returnKeyType="search"
              />
              {isSearchingUsers && <ActivityIndicator size="small" color={PageColors.textWhite} style={{ marginLeft: 10 }} />}
            </View>
            ...
            {!isSearchingUsers && searchResults.length === 0 && searchQuery.trim() === "" && (
              <Text style={styles.emptyStateText}>Enter a username to search for members to invite or manage requests.</Text>
            )}
          </View>
        );
        ...
        return (
          <View style={styles.sectionCard}>
            ...
            {(questions.length > 0 || initialQuestionsLoaded) && (
              <TouchableOpacity style={[styles.saveButton, (isSavingAll || isLoadingQuestions) && styles.disabledButton]} onPress={handleSaveAllQuestionChanges} disabled={isSavingAll || isLoadingQuestions}>
                {isSavingAll ? <ActivityIndicator color={PageColors.textWhite} /> : <>
  ...
  if (isLoadingGroup || !authLoaded) {
    return (
      <View style={styles.loaderContainer}>
        ...
        <Text style={styles.loaderText}>{!authLoaded ? 'Authenticating...' : 'Loading Group Details...'}</Text>
      </View>
    );
  }
...
const styles = StyleSheet.create({
  ...
  header: {
    flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: 15, paddingVertical: 12,
    paddingTop: Platform.OS === "android" ? 40 : 50, borderBottomWidth: 1, borderBottomColor: "rgba(255,255,255,0.1)"
  },
  backButton: { padding: 5 },
  headerTitle: { color: PageColors.textWhite, fontSize: 20, fontWeight: "bold" },
  sectionCard: {
    backgroundColor: PageColors.cardBackground, borderRadius: 12, marginHorizontal: 15, marginTop: 20, padding: 15,
    shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.2, shadowRadius: 3, elevation: 3
  },
  sectionTitle: { fontSize: 18, fontWeight: "600", color: PageColors.textWhite, marginBottom: 15 },
  sectionHeaderWithAction: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 },
  searchInputContainer: { flexDirection: "row", alignItems: "center", marginBottom: 12 },
  searchInput: {
    flex: 1, backgroundColor: PageColors.inputBackground, color: PageColors.textWhite, paddingHorizontal: 15,
  ...
  invitedStatusText: { color: PageColors.textGray, fontSize: 12, fontStyle: 'italic', paddingHorizontal: 8 },
  ...
  questionInput: { flex: 1, color: PageColors.textWhite, paddingVertical: 10, fontSize: 15, marginRight: 5, minHeight: 40 },
  removeButton: { padding: 10 },
  addButton: { flexDirection: 'row', alignItems: 'center', paddingVertical: 6, paddingHorizontal: 10, backgroundColor: PageColors.inputBackground, borderRadius: 20 },
  addButtonText: { color: PageColors.primary, marginLeft: 5, fontWeight: '500', fontSize: 14 },
  saveButton: {
    backgroundColor: PageColors.primary, flexDirection: 'row', alignItems: 'center', justifyContent: 'center',
    paddingVertical: 12, borderRadius: 25, marginTop: 20, minHeight: 48
  },
  saveButtonText: { color: PageColors.textWhite, fontSize: 16, fontWeight: "bold" },
  disabledButton: { backgroundColor: PageColors.disabled },
  emptyStateText: { color: PageColors.textGray, textAlign: 'center', marginVertical: 15, fontSize: 14, fontStyle: 'italic', paddingHorizontal: 10 },
  primaryButton: {
    backgroundColor: PageColors.primary, paddingVertical: 14, paddingHorizontal: 30, borderRadius: 25,
    alignItems: "center", justifyContent: "center", width: "80%", marginTop: 20, minHeight: 50
  },
