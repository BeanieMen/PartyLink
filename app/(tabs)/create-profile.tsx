...
const CreateProfileScreen = () => {
  ...
  const handleSaveProfile = async () => {
    ...
    if (age.trim()) {
      ...
      if (isNaN(ageNum) || ageNum <= 10 || ageNum > 100) {
        Alert.alert(
          'Validation Error',
          'Please enter a valid age (e.g., 11-99), or leave it blank.',
        )
        setLoading(false)
        return
      }
  ...
  const privateInfoContent = (
    <>
      ...
      <Text style={styles.popupListItem}>• Users you are friends with.</Text>
...
const styles = StyleSheet.create({
  ...
  privateToggleInnerContainer: {
    // For checkbox and text
    flexDirection: 'row',
    alignItems: 'center',
  },
  checkbox: {
